...
function authenticate(passport, name, options, callback) {
  ...
  return function passportAuthenticate(ctx, next) {
    ...
    return p.then(cont => {
      ...
      // UPD: also call next if preventRedirect option is enabled, and location header is set
      // this may be useful, for example, for json rest APIs
      if (cont !== false || (options.preventRedirect && ctx.response.get('location'))) {
        if (next) {
          return next()
        }
      }
